...
export const LiveCategoryView = forwardRef<HTMLDivElement, LiveCategoryViewProps>(
  ({ categoryName, channels, onBack, onChannelClick }, ref) => {
    ...
    return (
      <div ref={ref} tabIndex={-1} className="w-full h-full flex flex-col focus:outline-none bg-background-primary">
        <header className="flex items-center p-4 border-b border-border-primary bg-background-secondary flex-shrink-0 z-10">
          <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 font-semibold text-text-primary bg-background-glass rounded-md hover:bg-primary/20 focus:outline-none focus:ring-2 focus:ring-primary-focus">
                ...
                return (
                  <div key={virtualItem.key} className="absolute top-0 left-0 w-full" style={{ height: `${virtualItem.size}px`, transform: `translateY(${virtualItem.start}px)` }}>
                    <div 
                      ...
                      className={`w-full h-full flex items-center justify-between px-4 text-left cursor-pointer transition-colors focus:outline-none ${isSelected ? 'bg-primary text-on-primary' : 'hover:bg-background-glass focus:bg-background-glass'}`}
                    >
